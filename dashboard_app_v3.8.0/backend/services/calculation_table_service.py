...
COLUMNS_TO_REMOVE = [
    ...
    "01i14FIC05", "01i14FIC06", "01i14FIC07", "01i14FIC08",
    "01i14FI01",
]
...
def filter_base_table_columns(columns):
    ...
    return [col for col in columns if col not in EXCLUDED_SET and col not in COLUMNS_TO_REMOVE]
...
def reconstruct_base_table(df, carbon=None, molecular_weight=None):
    ...
    date_col = "DateTime" if "DateTime" in df.columns else ("Date/Time" if "Date/Time" in df.columns else "DateTime")
    reactor_col = "Reactor" if "Reactor" in df.columns else ("Sampled_reactor" if "Sampled_reactor" in df.columns else "Reactor")
    ...
    df["N2[SLPH]"] = [
        pd.to_numeric(df.loc[idx, col], errors="coerce") if pd.notna(col) and col in df.columns else None
    ...
    derived_cols = [
        "ESTD_H2", "ESTD_N2", "N2[SLPH]", "Temperature", "Pressure",
        "Naphtha_online", "H2_out", "Naphtha_out", "Carbon_out", "Naphtha_in", "Naphtha/H2"
    ]
...
def extract_base_table_data(online_data, carbon=None, molecular_weight=None):
    ...
    reconstructed_df = reconstruct_base_table(df, carbon=carbon, molecular_weight=molecular_weight)
...
def process_base_table_file(file_path, carbon=None, molecular_weight=None):
    ...
    if rows and all(str(r.get("Reactor", "0")).strip() in ("0", "", "None") for r in rows[:10]):
        try:
            from flask import current_app
            ...
            if r_dir and Path(r_dir).exists():
                ...
                if reactor_res and reactor_res.get("rows"):
                    online_data["rows"] = apply_reactor_map_to_online_rows(rows, reactor_res["rows"])
        except Exception:
            pass

    return extract_base_table_data(online_data, carbon=carbon, molecular_weight=molecular_weight)
