...
const columnLabels = {
    ...
    H2_out: 'H2_out[mol%]',
    Naphtha_out: 'Naphtha_out[mol%]',
    Carbon_out: 'Carbon_out[mol%]',
    Naphtha_in: 'Naphtha_in[mol%]',
...
function getComponentListRows() {
    ...
    if (!saved) return [];
    if (Array.isArray(saved.r2)) return saved.r2;
    if (Array.isArray(saved.table2_rows)) return saved.table2_rows;
    if (Array.isArray(saved.table2Rows)) return saved.table2Rows;
    return [];
}
