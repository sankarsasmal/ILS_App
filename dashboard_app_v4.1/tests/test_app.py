...
def test_endpoints():
    ...
    assert c.get("/custom-table").status_code == 200
    assert c.get("/custom_table").status_code == 200
    assert c.get("/frontend/custom_table.html").status_code == 200
    assert c.get("/plots").status_code == 200
    assert c.get("/frontend/plots.html").status_code == 200
    assert c.get("/api/health").json["version"] == "4.1.0"
