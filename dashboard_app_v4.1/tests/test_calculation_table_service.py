...
def test_calculate_yield_table_scales_component_list_values_and_preserves_blanks():
    ...
    assert result["rows"][0]["DateTime"] == "2026-09-04 16:20:46"
    assert result["rows"][0]["TOS(h)"] == 0.0
    assert result["rows"][0]["C3 & Below"] == 20
    assert result["rows"][0]["n-Octane"] == ""
    assert result["rows"][0]["Unknown"] == 5
    assert result["rows"][0]["Gas"] == 20.0
    assert result["rows"][0]["C5+"] == 0.0
    assert result["rows"][0]["H2_Consumption"] == -80.0


def test_calculate_yield_table_cumulative_tos_h():
    carbon_rows = [
        {"DateTime": "2026-09-04 16:00:00", "C3 & Below": 10},
        {"DateTime": "2026-09-04 18:30:00", "C3 & Below": 10},
        {"DateTime": "2026-09-04 21:00:00", "C3 & Below": 10},
    ]
    base_rows = [
        {"DateTime": "2026-09-04 16:00:00", "Naphtha_out": 4, "Naphtha_in": 4},
        {"DateTime": "2026-09-04 18:30:00", "Naphtha_out": 4, "Naphtha_in": 4},
        {"DateTime": "2026-09-04 21:00:00", "Naphtha_out": 4, "Naphtha_in": 4},
    ]
    result = calculate_yield_table(
        carbon_rows, base_rows, carbon_columns=["DateTime", "C3 & Below"]
    )
    assert "TOS(h)" in result["columns"]
    assert result["rows"][0]["TOS(h)"] == 0.0
    # 18:30 - 16:00 = 2.5 hrs
    assert result["rows"][1]["TOS(h)"] == 2.5
    # 21:00 - 18:30 = 2.5 hrs, cumulative = 5.0 hrs
    assert result["rows"][2]["TOS(h)"] == 5.0


def test_calculate_yield_table_gas_c5_and_h2_consumption_formulas():
    result = calculate_yield_table(
        [
            {
    ...
    # Gas = 5 + 3 + 2 + 4 = 14.0
    assert row["Gas"] == 14.0
    # Range sum (C3 & Below to n-Octane) = 5 + 3 + 2 + 4 + 15 + 10 + 20 + 25 + 21 = 105.0
    # Unknown_C9 is outside range
    # C5+ = 105.0 - 14.0 = 91.0
    assert row["C5+"] == 91.0
    # H2_Consumption = 105.0 - 100.0 = 5.0
    assert row["H2_Consumption"] == 5.0


def test_calculate_yield_table_matches_date_and_hour_only():
    result = calculate_yield_table(
        [{"DateTime": "2026-09-04 16:17:59", "C3 & Below": 10}],
        [{"DateTime": "2026-09-04 16:05:01", "Naphtha_out": 6, "Naphtha_in": 3}],
        carbon_columns=["DateTime", "C3 & Below"],
    )

    assert result["rows"][0]["C3 & Below"] == 20


def test_calculate_yield_table_matches_across_hour_boundary_with_hour_plus_or_minus_one():
    result = calculate_yield_table(
        [{"DateTime": "05/09/2026 12:57", "C3 & Below": 10}],
        [{"DateTime": "2026-09-05 13:01:00", "Naphtha_out": 6, "Naphtha_in": 3}],
        carbon_columns=["DateTime", "C3 & Below"],
    )
...
def test_calculate_yield_table_ignores_minutes_when_hours_are_within_one():
    result = calculate_yield_table(
        [{"DateTime": "05/09/2026 16:59", "C3 & Below": 10}],
        [{"DateTime": "2026-09-05 17:59:00", "Naphtha_out": 6, "Naphtha_in": 3}],
        carbon_columns=["DateTime", "C3 & Below"],
    )
...
def test_calculate_yield_table_applies_hour_tolerance_on_same_date():
    result = calculate_yield_table(
        [{"DateTime": "2026-09-04 16:00:00", "C3 & Below": 10}],
        [
            {"DateTime": "2026-09-04 16:00:00", "Naphtha_out": 6, "Naphtha_in": 3},
            {"DateTime": "2026-09-04 17:00:00", "Naphtha_out": 9, "Naphtha_in": 3},
        ],
        carbon_columns=["DateTime", "C3 & Below"],
    )
...
def test_calculate_yield_table_reports_invalid_ratio_and_missing_datetime():
    invalid = calculate_yield_table(
        [{"DateTime": "2026-09-04 16:17:59", "C3 & Below": 1}],
        [{"DateTime": "2026-09-04 16:05:01", "Naphtha_out": "bad", "Naphtha_in": 2}],
        carbon_columns=["DateTime", "C3 & Below"],
    )
    missing = calculate_yield_table(
        [{"DateTime": "b", "C3 & Below": 1}],
        [],
        carbon_columns=["DateTime", "C3 & Below"],
    )

    assert invalid["rows"][0]["C3 & Below"] == "ERROR: Invalid Naphtha_out [mol%]"
    assert invalid["rows"][0]["Gas"] == "ERROR: Invalid Naphtha_out [mol%]"
    assert invalid["rows"][0]["C5+"] == "ERROR: Invalid Naphtha_out [mol%]"
    assert invalid["rows"][0]["H2_Consumption"] == "ERROR: Invalid Naphtha_out [mol%]"

    assert missing["rows"][0]["C3 & Below"] == "ERROR: No matching DateTime"
    assert missing["rows"][0]["Gas"] == "ERROR: No matching DateTime"
    assert missing["rows"][0]["C5+"] == "ERROR: No matching DateTime"
    assert missing["rows"][0]["H2_Consumption"] == "ERROR: No matching DateTime"


def test_calculation_table_uses_offline_table_2_for_yield_table():
    from pathlib import Path

    base_dir = Path(__file__).resolve().parent.parent
    calc_js = (base_dir / "frontend/js/calculation_table.js").read_text(
        encoding="utf-8"
    )
    assert "Array.isArray(saved.r2)" in calc_js
    assert "Array.isArray(saved.c2)" in calc_js
    assert "Array.isArray(saved.r1)" not in calc_js
...
def test_extract_base_table_data():
    ...
    base_table = extract_base_table_data(
        online_mock, carbon=83.61, molecular_weight=88.55
    )
